%In this code we are generating the scenario when the asset price of BINARY
%option equals to strike price. We are observing the changes in option 
%price, deltaas, bank deposits and portfolios and we are plotting it.
...
for i = 1:N
    for j = 1:M
        ...
        V(j,i) = A*exp(-r*tau)*normcdf(-d2);
        Delta(j,i) = -(A*exp(-r*tau)*normpdf(d2))/(sigma*S(j,i)*sqrt(tau));
    end
end
for i = 1:M
    V(i,N+1) = A*hvsd(S(i,N+1)-K); % payoff at T
    %Delta(i,N+1) = (A*normpdf(d2))/(sigma*S(i,N+1)*sqrt(tau));
end
